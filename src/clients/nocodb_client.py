...
from clients.nocodb_client_config import (
    NOCODB_URL, NOCODB_API_TOKEN, NOCODB_PROJECT_ID,
...
class NocoDBJournalClient(AbstractJournalClient):
    ...
    def _get_journal_table_meta(self):
        ...
        print(f"Journal table '{NOCODB_JOURNAL_TABLE_NAME}' not found. Attempting to create...")
    ...
    def _get_detailed_table_meta(self, table_id):
        ...
        detailed_meta_url = f"{self.nocodb_url}/api/v2/meta/tables/{table_id}"
        try:
    ...
    def _journal_entry_to_nocodb_record(self, entry: JournalEntry) -> Dict[str, Any]:
        ...
        if entry.id is not None: record_fields[NOCODB_FIELD_NAMES["id"]] = entry.id
        if entry.entry_at is not None: record_fields[NOCODB_FIELD_NAMES["entry_at"]] = entry.entry_at.isoformat()
        if entry.timezone is not None: record_fields[NOCODB_FIELD_NAMES["timezone"]] = entry.timezone
        if entry.created_at is not None: record_fields[NOCODB_FIELD_NAMES["created_at"]] = entry.created_at.isoformat()
        if entry.modified_at is not None: record_fields[NOCODB_FIELD_NAMES["modified_at"]] = entry.modified_at.isoformat()
        if entry.text_content is not None: record_fields[NOCODB_FIELD_NAMES["text_content"]] = entry.text_content
        if entry.rich_text_content is not None: record_fields[NOCODB_FIELD_NAMES["rich_text_content"]] = entry.rich_text_content
        if entry.title is not None: record_fields[NOCODB_FIELD_NAMES["title"]] = entry.title
        if entry.tags: record_fields[NOCODB_FIELD_NAMES["tags"]] = ", ".join(entry.tags) # NocoDB 'varchar' for tags
        if entry.notebook is not None: record_fields[NOCODB_FIELD_NAMES["notebook"]] = entry.notebook
        ...
        if entry.mood_label is not None: record_fields[NOCODB_FIELD_NAMES["mood_label"]] = entry.mood_label
        if entry.mood_score is not None: record_fields[NOCODB_FIELD_NAMES["mood_score"]] = entry.mood_score
        if entry.activities: record_fields[NOCODB_FIELD_NAMES["activities"]] = ", ".join(entry.activities) # NocoDB 'varchar' for activities
        if entry.location_lat is not None: record_fields[NOCODB_FIELD_NAMES["location_lat"]] = entry.location_lat
        if entry.location_lon is not None: record_fields[NOCODB_FIELD_NAMES["location_lon"]] = entry.location_lon
        if entry.location_name is not None: record_fields[NOCODB_FIELD_NAMES["location_name"]] = entry.location_name
        if entry.location_address is not None: record_fields[NOCODB_FIELD_NAMES["location_address"]] = entry.location_address
        if entry.location_altitude is not None: record_fields[NOCODB_FIELD_NAMES["location_altitude"]] = entry.location_altitude
        if entry.weather_temperature is not None: record_fields[NOCODB_FIELD_NAMES["weather_temperature"]] = entry.weather_temperature
        if entry.weather_condition is not None: record_fields[NOCODB_FIELD_NAMES["weather_condition"]] = entry.weather_condition
        if entry.weather_humidity is not None: record_fields[NOCODB_FIELD_NAMES["weather_humidity"]] = entry.weather_humidity
        if entry.weather_pressure is not None: record_fields[NOCODB_FIELD_NAMES["weather_pressure"]] = entry.weather_pressure
        if entry.device_name is not None: record_fields[NOCODB_FIELD_NAMES["device_name"]] = entry.device_name
        if entry.step_count is not None: record_fields[NOCODB_FIELD_NAMES["step_count"]] = entry.step_count
        # Media attachments will be stored as JSON string in a longtext field
        if entry.media_attachments: record_fields[NOCODB_FIELD_NAMES["media_attachments"]] = json.dumps(entry.media_attachments)
        if entry.source_app_name is not None: record_fields[NOCODB_FIELD_NAMES["source_app_name"]] = entry.source_app_name
        if entry.source_original_id is not None: record_fields[NOCODB_FIELD_NAMES["source_original_id"]] = entry.source_original_id
        if entry.source_imported_at is not None: record_fields[NOCODB_FIELD_NAMES["source_imported_at"]] = entry.source_imported_at.isoformat()
        if entry.source_raw_data is not None: record_fields[NOCODB_FIELD_NAMES["source_raw_data"]] = json.dumps(entry.source_raw_data) # Store raw data as JSON string
    ...
    def register_entries(self, entries: List[JournalEntry]) -> List[Any]:
        table_meta = self._get_journal_table_meta()
        table_uuid = table_meta["uuid"]
        nocodb_records = [self._journal_entry_to_nocodb_record(entry) for entry in entries]
    ...
    def create_table(self, table_name, columns_definition):
        ...
        try:
            response = requests.post(create_table_url, headers=self.headers, json=payload)
            response.raise_for_status()
    ...
    def add_column_to_table(self, table_id, column_definition):
        ...
        add_column_url = f"{self.nocodb_url}/api/v2/meta/tables/{table_id}/columns"
        print(f"Attempting to add column '{column_definition.get("title")}' to table ID '{table_id}'...")
        try:
            response = requests.post(add_column_url, headers=self.headers, json=column_definition)
            print(response.text)
            response.raise_for_status()
            print(f"Column '{column_definition.get("title")}' added successfully.")
            return response.json()
        except requests.exceptions.RequestException as e:
            return self._handle_request_error(e, f"adding column {column_definition.get("title")}")

    def upload_file(self, file_path):
        """Uploads a file to NocoDB storage and returns its metadata."""
        upload_url = f"{self.nocodb_url}/api/v2/storage/upload"
        try:
    ...
    def create_attachment_record(self, attachment_table_id, file_metadata, order):
        ...
        create_url = f"{self.nocodb_url}/api/v2/tables/{attachment_table_id}/records"
        attachment_data = {
            ...
            "Attachment": [
                {
                    "url": "https://some-s3-server.com/nc/uploads/2023/10/16/some-key/3niqHLngUKiU2Hupe8.jpeg", # Placeholder
        ...
        try:
            response = requests.post(create_url, headers=self.headers, json=attachment_data)
            print(response.text)
            response.raise_for_status()
            return response.json()
        except requests.exceptions.RequestException as e:
            return self._handle_request_error(e, f"creating attachment record for {file_metadata.get('title')}")

    def link_attachment_to_journal_entry(self, attachment_table_id, attachment_record_id, link_column_id, journal_entry_id):
        """Links an attachment record to a journal entry."""
        link_url = f"{self.nocodb_url}/api/v2/tables/{attachment_table_id}/links/{link_column_id}/records/{attachment_record_id}"

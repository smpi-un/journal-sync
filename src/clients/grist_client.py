...
def _grist_record_to_journal_entry(grist_record: dict) -> JournalEntry:
    ...
    for grist_key, value in fields.items():
        ...
        try:
            ...
            print(
                f"Warning: Could not convert field '{grist_key}' with value '{value}'. Error: {e}"
            )
...
class GristJournalClient(AbstractJournalClient):
    ...
    def create_table(self):
        ...
        payload = {
            "tables": [
                {"id": self.journal_table_name, "columns": GRIST_JOURNAL_TABLE_COLUMNS}
            ]
        }
        try:
            self._make_request("POST", path, json_data=payload)
            print(
                f"Successfully sent request to create table '{self.journal_table_name}'."
            )
    ...
    def register_entries(self, entries: List[JournalEntry]) -> List[Any]:
        records_to_create = [
            {"fields": _journal_entry_to_grist_record(entry)} for entry in entries
        ]
    ...
    def download_journal_entries(self) -> List[JournalEntry]:
        ...
        try:
            ...
            if response_data and "records" in response_data:
                return [
                    _grist_record_to_journal_entry(rec)
                    for rec in response_data["records"]
                ]
            return []

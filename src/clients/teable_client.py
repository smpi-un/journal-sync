...
class TeableJournalClient(AbstractJournalClient):
    ...
    def _ensure_tables_exist(self):
        ...
        if not journal_table:
            ...
            )  # Import here to avoid circular dependency if config imports client
        ...
        if not self.attachment_field_id:
            print(
                f"Warning: Could not find the field ID for '{TEABLE_FIELD_NAMES['media_attachments']}'. File uploads will fail."
            )
        else:
            print(
                f"Found Attachment Field '{TEABLE_FIELD_NAMES['media_attachments']}' with ID: {self.attachment_field_id}"
            )
    ...
    def _verify_primary_key_setting(self, table_id: str, field_name: str):
        ...
        if not fields:
            print(
                f"Warning: Could not retrieve fields for table ID {table_id} to verify primary key."
            )
            return
        ...
        if not is_primary_found:
            print(
                f"Warning: Field '{field_name}' in table ID {table_id} is NOT marked as primary key in Teable."
            )
            print(
                "This might lead to unexpected behavior regarding unique identification."
            )
    ...
    def _journal_entry_to_teable_record(
        self, entry: JournalEntry, include_attachments=False
    ) -> dict[str, Any]:
        record_fields = {}

        # Explicitly map JournalEntry fields to Teable column names and types
        if entry.id is not None:
            record_fields[TEABLE_FIELD_NAMES["id"]] = entry.id
        # print(entry.id) # Reduce noise
        if entry.entry_at is not None:
            record_fields[TEABLE_FIELD_NAMES["entry_at"]] = entry.entry_at.isoformat()
        if entry.timezone is not None:
            record_fields[TEABLE_FIELD_NAMES["timezone"]] = entry.timezone
        if entry.created_at is not None:
            record_fields[TEABLE_FIELD_NAMES["created_at"]] = (
                entry.created_at.isoformat()
            )
        if entry.modified_at is not None:
            record_fields[TEABLE_FIELD_NAMES["modified_at"]] = (
                entry.modified_at.isoformat()
            )
        if entry.text_content is not None:
            record_fields[TEABLE_FIELD_NAMES["text_content"]] = entry.text_content
        if entry.rich_text_content is not None:
            record_fields[TEABLE_FIELD_NAMES["rich_text_content"]] = (
                entry.rich_text_content
            )
        if entry.title is not None:
            record_fields[TEABLE_FIELD_NAMES["title"]] = entry.title
        if entry.tags:
            record_fields[TEABLE_FIELD_NAMES["tags"]] = ", ".join(
                entry.tags
            )  # Teable 'singleLineText' for tags
        if entry.notebook is not None:
            record_fields[TEABLE_FIELD_NAMES["notebook"]] = entry.notebook
        ...
        if entry.mood_label is not None:
            record_fields[TEABLE_FIELD_NAMES["mood_label"]] = entry.mood_label
        if entry.mood_score is not None:
            record_fields[TEABLE_FIELD_NAMES["mood_score"]] = entry.mood_score
        if entry.activities:
            record_fields[TEABLE_FIELD_NAMES["activities"]] = ", ".join(
                entry.activities
            )  # Teable 'singleLineText' for activities
        if entry.location_lat is not None:
            record_fields[TEABLE_FIELD_NAMES["location_lat"]] = entry.location_lat
        if entry.location_lon is not None:
            record_fields[TEABLE_FIELD_NAMES["location_lon"]] = entry.location_lon
        if entry.location_name is not None:
            record_fields[TEABLE_FIELD_NAMES["location_name"]] = entry.location_name
        if entry.location_address is not None:
            record_fields[TEABLE_FIELD_NAMES["location_address"]] = (
                entry.location_address
            )
        if entry.location_altitude is not None:
            record_fields[TEABLE_FIELD_NAMES["location_altitude"]] = (
                entry.location_altitude
            )
        if entry.weather_temperature is not None:
            record_fields[TEABLE_FIELD_NAMES["weather_temperature"]] = (
                entry.weather_temperature
            )
        if entry.weather_condition is not None:
            record_fields[TEABLE_FIELD_NAMES["weather_condition"]] = (
                entry.weather_condition
            )
        if entry.weather_humidity is not None:
            record_fields[TEABLE_FIELD_NAMES["weather_humidity"]] = (
                entry.weather_humidity
            )
        if entry.weather_pressure is not None:
            record_fields[TEABLE_FIELD_NAMES["weather_pressure"]] = (
                entry.weather_pressure
            )
        if entry.device_name is not None:
            record_fields[TEABLE_FIELD_NAMES["device_name"]] = entry.device_name
        if entry.step_count is not None:
            record_fields[TEABLE_FIELD_NAMES["step_count"]] = entry.step_count
        # Attachments are handled in register_entries
        if entry.source_app_name is not None:
            record_fields[TEABLE_FIELD_NAMES["source_app_name"]] = entry.source_app_name
        if entry.source_original_id is not None:
            record_fields[TEABLE_FIELD_NAMES["source_original_id"]] = (
                entry.source_original_id
            )
        if entry.source_imported_at is not None:
            record_fields[TEABLE_FIELD_NAMES["source_imported_at"]] = (
                entry.source_imported_at.isoformat()
            )
        if entry.source_raw_data is not None:
            record_fields[TEABLE_FIELD_NAMES["source_raw_data"]] = json.dumps(
    ...
    def register_entries(self, entries: list[JournalEntry]) -> list[Any]:
        ...
        for entry in entries:
            if entry.media_attachments:
                ...
                records_to_create_without_attachments.append(
                    self._journal_entry_to_teable_record(entry)
                )
        ...
        if records_to_create_without_attachments:
            print(
                f"Registering {len(records_to_create_without_attachments)} entries without attachments..."
            )
            try:
                response = self.create_records(
                    table_id, records_to_create_without_attachments
                )
        ...
        if entries_with_attachments:
            if not self.attachment_field_id:
                print(
                    "Error: Attachment field ID is not available. Cannot upload files."
                )
                ...
                for entry in entries_with_attachments:
                    ...
                    try:
                        ...
                        print(
                            f"Error creating record for entry {entry.id} (attachments skipped): {e}"
                        )
                return all_results

            print(
                f"Registering {len(entries_with_attachments)} entries with attachments one by one..."
            )
            for entry in entries_with_attachments:
                ...
                try:
                    ...
                    if not created_records:
                        print(
                            f"Failed to create record for entry {entry.id}, skipping attachment upload."
                        )
                        continue
                    ...
                    print(
                        f"Uploading {len(entry.media_attachments)} attachments for record {record_id}..."
                    )
                    for attachment in entry.media_attachments:
                        ...
                        if file_path and os.path.exists(file_path):
                            try:
                                ...
                                print(
                                    f"  Error uploading file {file_path} for record {record_id}: {e}"
                                )
                        else:
                            print(
                                f"  Warning: Attachment file not found or path is missing: {file_path}"
                            )
    ...
    def get_existing_entries_with_modified_at(self) -> dict[str, datetime]:
        ...
        if all_records and "records" in all_records:
            for record in all_records["records"]:
                ...
                modified_at_str = record["fields"].get(
                    TEABLE_FIELD_NAMES["modified_at"]
                )
    ...
    def update_entries(self, entries: list[JournalEntry]) -> list[Any]:
        table_id = self._get_journal_table_id()
        teable_records = [
            self._journal_entry_to_teable_record(entry) for entry in entries
        ]
        response = self.update_records(table_id, teable_records)
        return response

    def _make_request(self, method, path, json_data=None):
        # path.lstrip('/') is important as urljoin treats paths starting with '/' as absolute
    ...
    def create_table(self, table_name, fields):
        ...
        return self._make_request(
            "POST", f"/base/{self.base_id}/table", json_data=payload
        )
    ...
    def create_records(self, table_id, records):
        ...
        print(records)
        return self._make_request(
            "POST", f"/table/{table_id}/record", json_data={"records": records}
        )
    ...
    def update_records(self, table_id, records):
        ...
        for record in records:
            record_id = record.get(
                "id"
            )  # Assuming 'id' is present in the record payload
            if not record_id:
            ...
            try:
                response = self._make_request(
                    ...
                    json_data={"fields": record["fields"]},
                )
    ...
    def upload_file(self, table_id, record_id, field_id, file_path):
        ...
        # Construct the relative path safely
        relative_path = (
        ...
        headers = {"Authorization": f"Bearer {TEABLE_API_TOKEN}"}

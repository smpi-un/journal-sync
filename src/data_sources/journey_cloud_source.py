...
class JourneyCloudDataSource(AbstractJournalDataSource):
    ...
    def _parse_journey_cloud_entry(self, raw_entry: Dict[str, Any], entry_dir_path: str) -> JournalEntry:
        ...
        # --- 時間情報 ---
        entry_at = parse_dt(raw_entry.get("dateOfJournal")) or parse_dt(raw_entry.get("createdAt")) or datetime.now() # Fallback to now if no date
        ...
        # --- 位置情報 (フラット化) ---
        location_lat = raw_entry.get("location", {}).get("latitude") if isinstance(raw_entry.get("location"), dict) else None
        location_lon = raw_entry.get("location", {}).get("longitude") if isinstance(raw_entry.get("location"), dict) else None
        location_name = raw_entry.get("location", {}).get("name") if isinstance(raw_entry.get("location"), dict) else None
        location_address = raw_entry.get("address")
        location_altitude = raw_entry.get("location", {}).get("altitude") if isinstance(raw_entry.get("location"), dict) else None

        # --- 天気情報 (フラット化) ---
        weather_temperature = raw_entry.get("weather", {}).get("temperature") if isinstance(raw_entry.get("weather"), dict) else None
        weather_condition = raw_entry.get("weather", {}).get("condition") if isinstance(raw_entry.get("weather"), dict) else None
        weather_humidity = raw_entry.get("weather", {}).get("humidity") if isinstance(raw_entry.get("weather"), dict) else None
        weather_pressure = raw_entry.get("weather", {}).get("pressure") if isinstance(raw_entry.get("weather"), dict) else None
    ...
    def _load_journal_entry_from_json(self, entry_dir_path: str, entry_id: str) -> Optional[Dict[str, Any]]:
        """

...
def main(source_data_path: str, client_type: str = "teable"):
    ...
    if client_type == "teable":
        ...
        raise ValueError(f"Unknown client type: {client_type}. Choose 'teable' or 'nocodb'.")

    # 3. Initialize and run Journal Manager
    manager = JournalManager(data_source=data_source, journal_client=journal_client)
    
    print(f"Starting import and registration process using {type(data_source).__name__} and {type(journal_client).__name__}.")
